...
from images import YCbCrImg, RGBImg
...
if __name__ == '__main__':
    ...
    imgycbcr = YCbCrImg()
    imgycbcr.setDataFromRGBImg(imgrgb)
    
    # image originale
    fig = plt.figure(num_fig)
    num_fig += 1
    imgycbcr.setTitle("Image originale")
    imgycbcr.draw(plt.gca())
    ...
    if draw_channels:
        fig = plt.figure(num_fig)
        num_fig += 1
        imgycbcr.drawEachChannel(fig, "Canaux Y, Cb et Cr de l'image originale")
    
    
    # histogramme image original
    if (draw_histo):
        num_fig = drawHisto(imgycbcr, "l'image originale", xminxmaxfixed, num_fig)
    ...
    if (strechtImg):
        ...
        if draw_channels:
            fig = plt.figure(num_fig)
            num_fig += 1
            img2.drawEachChannel(fig, "Canaux Y, Cb et Cr de l'image après étalement")  
